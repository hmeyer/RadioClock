...
void reportError(unsigned int c) {
    /*
...
Returns:  last receive error
data:  received byte from ringbuffer
**************************************************************************/
...
{    
    ...
    *data = UART_RxBuf[tmptail];
    ...
    status = UART_LastRxError;
    UART_LastRxError = 0;
    if (status) {
    	reportError(*data); return status;
    }
    if ( status & UART_NO_DATA ) return status;
    return status;
}/* uart_getc */
